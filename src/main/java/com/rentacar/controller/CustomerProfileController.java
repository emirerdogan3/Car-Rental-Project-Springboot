...
@PreAuthorize("hasRole('CUSTOMER')") // Sadece müşteriler kendi profillerini yönetebilir
...
public class CustomerProfileController {
    ...
    public String viewProfile(Model model, @AuthenticationPrincipal UserDetails currentUser) {
        try {
            System.out.println("DEBUG: Customer profile request for user: " + currentUser.getUsername());
            ...
            if (userProfile != null) {
                System.out.println("DEBUG: Profile details - firstName: " + userProfile.getFirstName() + 
                                 ...
                                 ", totalRentals: " + userProfile.getTotalRentals());
            }
            model.addAttribute("userProfile", userProfile);
            model.addAttribute("changePasswordDTO", new ChangePasswordDTO()); // For password change form
            return "customer/profile"; // Thymeleaf template path
        } catch (Exception e) {
            System.out.println("DEBUG: Error in customer profile: " + e.getMessage());
            e.printStackTrace();
    ...
    public String updateProfile(@Valid @ModelAttribute("userProfile") UserProfileDTO userProfileDTO,
        ...
        if (bindingResult.hasErrors()) {
            model.addAttribute("changePasswordDTO", new ChangePasswordDTO()); // Keep password DTO in model
            return "customer/profile";
        }
        try {
    ...
    public String changePassword(@Valid @ModelAttribute("changePasswordDTO") ChangePasswordDTO changePasswordDTO,
        ...
        if (bindingResult.hasErrors()) {
            UserProfileDTO userProfile = userService.getCurrentUserProfile(currentUser);
            model.addAttribute("userProfile", userProfile); // Keep profile DTO in model
            return "customer/profile";
        }
